    ...
    switch (payment_method) {
        ...
        responseData = {
          ...
          bank_accounts: bankAccounts.map((bank) => ({
            ...
            qr_code: bank.qr_code
              ? `${req.protocol}://${req.get("host")}/uploads/${bank.qr_code}`
              : null,
          })),
          order_code: orderCode,
          transfer_content: transferContent,
      ...
      case "momo":
        try {
          ...
          const momoResponse = await MomoUtil.createPayment(
            order.id,
            order.total_amount,
            orderInfo
          );
      ...
      case "vnpay":
        try {
          ...
          const vnpayResponse = VNPayUtil.createPayment(
            order.id,
            orderAmount,
            orderInfo,
            ipAddr
...
exports.vnpayCallback = async (req, res, next) => {
  try {
    ...
    if (payments && payments.length > 0) {
      ...
      const oldPaymentData = payments[0].payment_data || {};
    ...
    res.redirect(
      `http://localhost:3000/payment/vnpay/return?order_id=${orderId}&status=success&vnp_ResponseCode=${vnp_ResponseCode}&vnp_TxnRef=${txnRef}`
    );
  } catch (error) {
...
exports.generateBankQR = async (req, res, next) => {
  try {
    ...
    const qrContent = `https://img.vietqr.io/image/${bankCode}-${
      bank.account_number
    }-compact.png?amount=${amount}&addInfo=${encodeURIComponent(
      transferContent
    )}&accountName=${encodeURIComponent(bank.account_holder)}`;

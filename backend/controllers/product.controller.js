  ...
  try {
    ...
    products.forEach((product) => {
      ...
      if (product.images) {
        ...
        product.images = images.map((img) => `${req.protocol}://${req.get("host")}/uploads/${img}`)
      }
    })
...
exports.deleteProduct = async (req, res, next) => {
  try {
    ...
    // Xóa hình ảnh gallery nếu tồn tại
...
exports.uploadGallery = async (req, res, next) => {
  try {
    ...
    res.json({
      ...
      data: {
        images: newImages.map((img) => `${req.protocol}://${req.get("host")}/uploads/${img}`),
      },
    })
  } catch (error) {
